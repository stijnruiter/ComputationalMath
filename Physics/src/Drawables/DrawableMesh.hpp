...
class DrawableMesh : public Render::DrawableObject
{
public:
    DrawableMesh(const Geometry::Mesh2D& mesh);
    DrawableMesh(const Geometry::Mesh2D& mesh, const std::vector<float>& values);
    DrawableMesh(const Geometry::Mesh2D& mesh, const LinearAlgebra::ColumnVector<float>& values);
...
protected:
    ...
    std::unique_ptr<Render::VertexBuffer> m_valuesBuffer;
    std::unique_ptr<Render::IndexBuffer<Geometry::TriangleElement>> m_triangleBuffer;
    std::unique_ptr<Render::IndexBuffer<Geometry::LineElement>> m_interiorEdgeBuffer;
    std::unique_ptr<Render::IndexBuffer<Geometry::LineElement>> m_boundaryEdgeBuffer;
};